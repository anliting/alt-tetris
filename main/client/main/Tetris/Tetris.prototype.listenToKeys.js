    ...
    var keyevents=function(){
        ...
        if(tetris.keys[40]){    // down arrow: soft drop
            if(tetris.times_key[40]%4==0)
                tetris._tetromino.softdrop()
        }
        if(tetris.keys[67]){    // c: hold
            if(tetris.times_key[67]%8==0)
                tetris._board_hold.hold()
            tetris._board_hold.update_html()
        }
        ...
        if(tetris.keys[90]){    // z: 逆時鐘轉
            if(tetris.times_key[90]%8==0)
                tetris._tetromino.rotate(0)
        }
        for(let i=0;i<128;i++)
            if(tetris.keys[i])
                tetris.times_key[i]++
        tetris._tetromino.update_html()
